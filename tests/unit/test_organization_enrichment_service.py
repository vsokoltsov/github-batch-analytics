...
from gba.services.enrichment import organization
...
def test_organization_enrichment_pipeline_build_source_uses_injected_source_factory() -> (
    None
):
    def source_factory(
        input_path: str,
    ...
    pipeline = organization.OrganizationEnrichmentPipeline(
        output_bucket="bronze-bucket",
        input_path="s3a://silver/org_candidates/dt=2026-04-01/hr=7/",
        dt="2026-04-01",
        hr=7,
    ...
    assert pipeline.build_source() == (
        "s3a://silver/org_candidates/dt=2026-04-01/hr=7/",
        "ghp_test",
        "2026-04-01",
        7,
    )
...
def test_organization_enrichment_pipeline_output_paths_are_partitioned() -> None:
    pipeline = organization.OrganizationEnrichmentPipeline(
        output_bucket="bronze-bucket",
        input_path="s3a://silver/org_candidates/dt=2026-04-01/hr=7/",
        dt="2026-04-01",
...
def test_organization_enrichment_pipeline_run_uses_pipeline_lifecycle_and_returns_paths() -> (
    None
):
    ...
    enrichment_pipeline = organization.OrganizationEnrichmentPipeline(
        output_bucket="bronze-bucket",
        input_path="s3a://silver/org_candidates/dt=2026-04-01/hr=7/",
        dt="2026-04-01",
        hr=7,
        github_token="ghp_test",
        pipeline_name="org-pipeline",
    ...
    assert pipeline_kwargs["pipeline_name"] == "org-pipeline"
    assert pipeline_kwargs["pipelines_dir"] == "/tmp/custom-dlt"
    assert pipeline_kwargs["dataset_name"] == "org_dataset"
