...
class BuildCandidates:
    ...
    def _common_metrics(self, group_by: List[str], *columns) -> DataFrame:
        metrics = self.df.groupBy(*group_by).agg(
            ...
            F.sum(
                F.when(
                    F.lower(F.col("actor_login")).ilike("%[bot]"),
                    1
                ).otherwise(0)
            ).alias("bot_events"),
    ...
    def event_types_list(self) -> List:
        return [item.event_type for item in self.df.select('event_type').distinct().collect()]
...
def main() -> None:
    ...
    spark = SparkSession.builder.appName(f"gharchive-build-candidate-{args.type}").getOrCreate() # type: ignore[attr-defined]
